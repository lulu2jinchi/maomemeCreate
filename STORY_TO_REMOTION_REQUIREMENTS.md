...
- 同一个 scene / 同一个 `layout.groupId` 里，不能同时出现两个重复的同一个人物。
  - 生成时不得把同一角色复制两份站在同一画面里凑人数。
  - 同一画面中的角色身份必须彼此可区分，例如“我 / 新同事 / 客户”，不能写成两个“我”。
  - 如果剧情想表达同一角色的连续动作，应拆成前后镜头，或只保留一个人物，用字幕和表演补信息。
...
- 若字幕表示某个具体人物的心理活动、吐槽或对白：
  ...
  - 不要把字幕丢到离人物很远的画面另一侧
  - 这类文本轨默认应写 `speakerTrackId`，指向对应人物视频轨 id，供渲染时自动贴到人物上方
  - 正文字幕内容必须是该人物会直接说出口的话，不能写成剧情解释、画面描述或总结句
  - 正文字幕里禁止写 `角色名：台词`、`我：`、`老板：` 这类前缀；谁在说话应靠 `speakerTrackId` 和贴近人物的摆位表达
  - 除非用户明确要求手调，否则不要手写一个离人物很远的 `style.x/y`
...
- 正文字幕默认写成角色正在说的话，不要写成旁白总结、剧情简介或说明书。
- 正文字幕必须直接落在台词本身，不要写“某人说”“某人表示”“画面里他很委屈”这类描述层。
- 正文字幕禁止使用 `角色名：台词` 格式；说话人标识默认交给 `speakerTrackId`、字幕摆位和镜头关系处理。
...
- 避免书面化表达：
  ...
  - 不要写成新闻播报或剧情简介
  - 避免“某角色表示…”、“事情正在发生变化…”、“他意识到局面已不同”等旁白式写法
